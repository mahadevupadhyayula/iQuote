...
const repositories = {
  quotes: {
    ...
    replaceItems: vi.fn(
      async (_quoteId: string, items: Record<string, unknown>[]) => {
        storedQuote = {
          ...storedQuote,
    ...
    listByProduct: vi.fn(async () => [
      {
        ...
        source_name: "test",
        source_version: "v1",
        refreshed_at: freshInventoryTimestamp,
        metadata: {},
        updated_at: freshInventoryTimestamp,
      },
    ]),
...
describe("quote actions", () => {
  ...
  it("persists rep corrections in structured metadata and records before/after workflow summaries", async () => {
    const result = await applyRepCorrections({
      ...
      metadata: {
        extraction: { original_request_text: "Attempted overwrite" },
      },
      lines: [
        {
          sku: "SKU-NEW",
    ...
    expect(result.metadata.extraction).toEqual({
      original_request_text: "Original immutable request",
    });
    expect(result.metadata.requirements).toMatchObject({
      existing: true,
      delivery_window: "Q3",
      clarification_answers: { shipping: "Dock 4" },
    });
    expect(result.metadata.review).toMatchObject({
      product_candidates: { 1: "SKU-NEW" },
      rep_confirmation: { confirmed: true, confirmed_by: actorId },
    });
  ...
  it("saves drafts without advancing quote status", async () => {
    await saveQuoteDraft({
      quote_id: quoteId,
      actor_id: actorId,
      requirements: { note: "keep drafting" },
      metadata: {},
    });
  ...
  it("triggers pricing when final deterministic line inventory is applied and persists pricing metadata", async () => {
    ...
    const result = await selectFulfillment({
      quote_id: quoteId,
      actor_id: actorId,
      line_number: 1,
    });
    ...
    expect(result.quote.metadata).toMatchObject({
      pricing_status: "resolved",
      pricing_resolved: true,
      pricing_blockers: [],
      commercial_calculation: expect.any(Object),
    });
    expect(storedQuote.items[0]).toMatchObject({
      unit_price: 250,
      ...
      metadata: expect.objectContaining({
        ...
        product_confirmation: expect.objectContaining({
          confirmed: true,
          product_id: productId,
          confirmation_source: "use_recommended",
        }),
        pricing_resolved: true,
        price_application: expect.objectContaining({
          price_type: "list",
          source_name: "test",
        }),
      }),
    });
    expect(repositories.workflowEvents.record).toHaveBeenCalledWith(
