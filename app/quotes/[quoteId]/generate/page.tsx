...
export default async function GenerateQuotePage({ params }: { params: Promise<{ quoteId: string }> }) {
  ...
  return (
    <WorkspaceLayout currentStep="generate" status={quote.status} contentClassName="space-y-6">
      ...
      <div className="flex flex-wrap gap-3"><Link href={`/api/quotes/${quote.id}/pdf`} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">Generate PDF</Link><button className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700"><Send className="h-4 w-4" /> Send quote</button></div>
    </WorkspaceLayout>
  );
}
