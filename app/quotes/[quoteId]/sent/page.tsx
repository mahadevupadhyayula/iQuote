...
export default async function SentQuotePage({ params }: { params: Promise<{ quoteId: string }> }) {
  ...
  return (
    <WorkspaceLayout currentStep="generate-quote" status={quote.status} contentClassName="max-w-3xl space-y-6" dashboardAction={<DashboardButton mode="navigate" />}>
      ...
      <Link href="/quotes" className="font-semibold text-blue-600 hover:text-blue-800">Dashboard</Link>
    </WorkspaceLayout>
  );
}
