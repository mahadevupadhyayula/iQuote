...
import {
  createQuoteWorkspaceQueryService,
  type InternalQuoteWorkspaceViewModel,
...
const exceptionGroups = [
  {
    ...
    title: "Discount",
    icon: BadgePercent,
    codes: ["margin_policy_failed", "approval_pending", "approval_rejected"],
    empty: "Discount and margin are within policy.",
  },
  {
    key: "missing",
    title: "Missing information",
    icon: FileQuestion,
    codes: ["missing_required_information", "payment_terms_missing"],
    empty: "Required quote information is complete.",
  },
  {
    key: "pricing",
    title: "Pricing",
    icon: Tags,
    codes: ["missing_price", "stale_price", "pricing_exception"],
    empty: "Pricing is current and sellable.",
  },
  {
    key: "inventory",
    title: "Inventory",
    icon: PackageCheck,
    codes: ["unresolved_inventory", "stale_inventory"],
    empty: "Inventory has been resolved.",
  },
  {
    key: "product",
    title: "Product validity",
    icon: ShieldAlert,
    codes: ["invalid_product", "blocking_exception"],
    empty: "Products are active and valid.",
  },
] as const;
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      {exceptionGroups.map((group) => {
        ...
        const blockers = quote.readiness.blockers.filter((blocker) =>
          (group.codes as readonly string[]).includes(blocker.code),
        );
        return (
          <Card
            key={group.key}
          ...
          >
            ...
            <CardContent className="space-y-2 text-xs">
              {blockers.length > 0 ? (
                ...
                <p className="text-slate-500">{group.empty}</p>
              )}
            </CardContent>
          </Card>
        );
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg">
          <FileText className="h-5 w-5 text-blue-600" /> Requirements
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="rounded-lg bg-amber-50 p-3 text-sm text-amber-800">
          ...
          {quote.readiness.blockers.length} readiness items require attention
        </div>
      </CardContent>
    </Card>
  );
...
function QuoteConfigurationTable({
  quote,
}: {
  quote: InternalQuoteWorkspaceViewModel;
}) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg">
          <Settings2 className="h-5 w-5 text-blue-600" /> Quote Configuration
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Table>
              ...
              <TableRow key={line.id}>
                ...
                <TableCell>
                  <Badge className="bg-white text-slate-700">
                    {line.inventoryDecision ? "Recommended" : "Unresolved"}
                  </Badge>
                </TableCell>
                <TableCell
                  className={line.discountBps > 1000 ? "text-red-600" : ""}
                >
                  {percent(line.discountBps)}
                </TableCell>
                <TableCell
                  className={
                    line.marginFloorPasses
                      ? "text-emerald-600"
                      : "text-amber-600"
                  }
                >
                  {percent(line.grossMarginBps)}
                </TableCell>
                <TableCell>
                  <FulfillmentButton
                    quote={quote}
...
function InventoryRecommendation({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg">
          <PackageCheck className="h-5 w-5 text-blue-600" /> Inventory
          Recommendation
        </CardTitle>
      </CardHeader>
        ...
        {quote.lines.map((line) => (
          <div key={line.id} className="rounded-lg border p-3">
            ...
            <p className="mt-1 text-sm text-slate-600">
              {line.inventoryDecision
                ? "Inventory has a saved recommendation. Confirm or replace with the recommended fulfillment choice."
                : "Run the recommended fulfillment action to resolve inventory for this line."}
            </p>
            <div className="mt-3">
...
function ReadinessPanel({ quote }: { quote: InternalQuoteWorkspaceViewModel }) {
  const checks = quote.readiness.ready
    ? [
        "Requirements complete",
        "Product configuration valid",
        "Pricing current",
        "Inventory resolved",
        "Margin within policy",
        "Terms accepted",
      ]
    : quote.readiness.blockers.map((b) => b.message);
  const pendingApprovals = quote.approvalStatus.approvals.filter(
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg">
          <CheckCircle2 className="h-5 w-5 text-blue-600" /> Quote Readiness
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {checks.map((check, index) => (
          <div key={index} className="flex gap-2 text-sm">
            <CheckCircle2
              className={`h-5 w-5 ${quote.readiness.ready ? "text-emerald-600" : "text-amber-600"}`}
            />
            <span>{check}</span>
          </div>
        ))}
        {pendingApprovals.map((approval) => (
          <Link
            ...
            className="block rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700"
          >
            Open {approval.required_role.replaceAll("_", " ")} approval
          </Link>
        ))}
      </CardContent>
    </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="flex justify-between">
            <span>Subtotal</span>
            <span>{currency(quote.subtotalAmount, quote.currencyCode)}</span>
          </div>
          <div className="flex justify-between text-red-600">
            <span>Discount</span>
            <span>-{currency(quote.discountAmount, quote.currencyCode)}</span>
          </div>
          <div className="flex justify-between border-t pt-3 text-lg font-bold">
            <span>Total</span>
            <span>{currency(quote.totalAmount, quote.currencyCode)}</span>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Clock3 className="h-5 w-5 text-orange-600" /> SLA
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid gap-3 md:grid-cols-4">
          {quote.workflowEvents.slice(0, 8).map((event) => (
            <div key={event.id} className="border-l-2 border-blue-600 pl-3">
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600">
            Three-column workspace for resolving exceptions and generating
            customer-ready quotes.
          </p>
        </div>
      ...
      <WorkspaceGrid
        ...
        main={
          <>
            ...
            <InventoryRecommendation quote={quote} />
            <ActivityTimeline quote={quote} />
          </>
        }
        right={
          <>
            <ReadinessPanel quote={quote} />
            <SummaryAndSla quote={quote} />
            <Card>
              <CardHeader>
