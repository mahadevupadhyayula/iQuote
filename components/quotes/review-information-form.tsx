...
export function ReviewInformationForm({ quote }: Props) {
  ...
  return <form action={submit("continue")} className="space-y-6">
    {quote.reviewMetadata.manualEntry.enabled && <Alert className="border-amber-300 bg-amber-50"><AlertTriangle className="h-4 w-4 text-amber-600" /><AlertTitle>Manual-entry mode enabled</AlertTitle><AlertDescription>Extraction did not produce a complete validated request. Enter customer facts here; pricing and inventory are intentionally not performed on Review.</AlertDescription></Alert>}
