...
const mergeQuoteMetadata = (
  ...
  updates: {
    ...
    rep_confirmation?: Record<string, unknown>;
  ...
  extraction: {
    ...
    ...(updates.original_request_text &&
    !("original_request_text" in asObject(current.extraction))
      ? { original_request_text: updates.original_request_text }
      : {}),
  },
  requirements: {
    ...
    ...(updates.clarification_answers
      ? { clarification_answers: updates.clarification_answers }
      : {}),
  },
  review: {
    ...asObject(current.review),
    ...(updates.product_candidates
      ? { product_candidates: updates.product_candidates }
      : {}),
    ...(updates.rep_confirmation
      ? { rep_confirmation: updates.rep_confirmation }
      : {}),
  },
});
...
const lineTotals = (
  lines: Pick<
    ...
    "quantity" | "unit_price" | "discount_bps"
  >[],
) => {
  ...
  const discount = lines.reduce(
    (sum, line) =>
      sum +
      (line.quantity * line.unit_price * (line.discount_bps ?? 0)) / 10_000,
    0,
  );
  return {
...
export async function createQuoteDraft(input: CreateQuoteDraftActionInput) {
  ...
  await repositories.workflowEvents.record({
    ...
    from_status: null,
    to_status: "draft",
    payload: { action: "create_quote_draft" },
  });
  revalidatePath(quotePath(quote.id));
  return quote;
}

export async function extractAndBuildQuote(
  input: ExtractAndBuildQuoteActionInput,
) {
  ...
  const result = await extractionService.extractAndPersist({
    quoteId: data.quote_id,
    sourceText: data.source_text,
    actorId: data.actor_id ?? null,
  });
  const extractedLines =
    result.extraction?.requested_items
      .map((line) => ({
  ...
  const matches = await createProductMatchingService({
    productsRepository: repositories.products,
  }).matchLines(extractedLines);
  const items = await Promise.all(
    matches.map(async (match) => {
      const extracted = result.extraction?.requested_items.find(
        (line) => line.line_number === match.lineNumber,
      );
      const price = match.product
        ? await repositories.prices.findCurrentPrice(
            match.product.id,
      ...
      return toQuoteItems([
        {
          ...
          sku:
            match.product?.sku ?? extracted?.requested_sku.value ?? "UNMATCHED",
          description:
            match.product?.name ??
            extracted?.raw_item_description.value ??
            "Unmatched item",
          quantity: extracted?.quantity.value ?? 1,
          unit_price: price?.unit_price ?? 0,
          discount_bps: 0,
          metadata: { product_match: match },
        },
      ])[0];
    }),
  );
  ...
  const quote = await repositories.quotes.update(data.quote_id, {
    metadata: mergeQuoteMetadata(refreshed.metadata, {
      original_request_text: data.source_text,
    }),
  });
  await recordUpdate(quote, data.actor_id, "extract_and_build_quote", {
...
const persistRepCorrections = async (
  data: ApplyRepCorrectionsActionInput | SaveQuoteDraftActionInput,
  action: "apply_rep_corrections" | "save_quote_draft",
) => {
  const { repositories } = getContext();
  ...
  if (!latest)
    throw new Error(`Quote ${data.quote_id} was not found after corrections.`);
  const totals = lineTotals(latest.items);
  const updated = await repositories.quotes.update(data.quote_id, {
    customer_id:
      "customer_id" in data && data.customer_id
        ? data.customer_id
        : before.customer_id,
    opportunity_id:
      "opportunity_id" in data && data.opportunity_id !== undefined
        ? data.opportunity_id
  ...
  await recordUpdate(updated, data.actor_id, action, {
    before: quoteSummary(before),
    after: quoteSummary(after),
  });
  revalidatePath(quotePath(data.quote_id));
  return updated;
};

export async function applyRepCorrections(
  input: ApplyRepCorrectionsActionInput,
) {
  return persistRepCorrections(
    applyRepCorrectionsActionSchema.parse(input),
    "apply_rep_corrections",
  );
}
...
export async function selectFulfillment(input: SelectFulfillmentActionInput) {
  ...
  const targetItem = quote.items.find(
    (item) => item.line_number === data.line_number,
  );
  if (!targetItem?.product_id)
    throw new Error(
      `Quote line ${data.line_number} does not have a product to fulfill.`,
    );
  const product = await repositories.products.findById(targetItem.product_id);
  if (!product)
    throw new Error(`Product ${targetItem.product_id} was not found.`);
  const inventoryService = createInventoryService({
    inventoryRepository: repositories.inventory,
    productsRepository: repositories.products,
  });
  const alreadyApplied =
    targetItem.metadata.product_confirmed === true &&
    targetItem.metadata.product_confirmation &&
    Array.isArray(targetItem.metadata.selected_fulfillment) &&
    typeof targetItem.metadata.inventory_confirmed_at === "string" &&
    targetItem.metadata.pricing_resolved === true;
  if (alreadyApplied) {
    revalidatePath(quotePath(data.quote_id));
    ...
    return {
      quote,
      pricingStatus:
        typeof quote.metadata.pricing_status === "string"
          ? quote.metadata.pricing_status
          : "resolved",
      pricingBlockers: Array.isArray(quote.metadata.pricing_blockers)
        ? quote.metadata.pricing_blockers
        : [],
    };
  }
  ...
  if (fulfillment.length === 0 || inventoryDecision.blocked)
    throw new Error(
      inventoryDecision.reason ??
        `Inventory is not fulfillable for quote line ${data.line_number}.`,
    );
      ...
      ? {
          ...
          metadata: {
            ...
            product_confirmation: {
              ...
              confirmation_source: "use_recommended",
            },
            product_confirmed: true,
            inventory_decision: inventoryDecision,
            selected_inventory_decision: { ...inventoryDecision, fulfillment },
            selected_fulfillment: fulfillment,
            inventory_confirmed_at: confirmedAt,
          },
        }
      : item,
  );
  ...
  let updated = await repositories.quotes.update(data.quote_id, {
    metadata: {
      ...quote.metadata,
      fulfillment_selected_at: new Date().toISOString(),
    },
  });
  await recordUpdate(updated, data.actor_id, "select_fulfillment", {
  ...
  let pricingStatus =
    typeof updated.metadata.pricing_status === "string"
      ...
      : completion?.allInventorySelectionsApplied === false
        ...
        : completion?.allProductMatchesConfirmed === false
          ...
          : "not_started";
  let pricingBlockers = Array.isArray(updated.metadata.pricing_blockers)
    ? updated.metadata.pricing_blockers
    : [];
  if (latest && allInventoryConfirmed(latest.items)) {
    const pricing = await createQuotePricingResolutionService(
      repositories,
    ).resolveQuotePricing({
      quoteId: data.quote_id,
      actorId: data.actor_id ?? null,
...
export async function submitQuoteForApproval(
  ...
  const { createApprovalService } =
    await import("@/lib/services/approval-service");
  const evaluation = await createApprovalService(
    repositories.prices,
  ).evaluatePolicy({
    requestedDiscountBps: discountBps,
    projectedMarginBps: marginBps,
  });
  if (evaluation.requiredRole) {
    const existingApproval = await repositories.approvals.findPendingForRole(
      data.quote_id,
      evaluation.requiredRole,
    );
    if (!existingApproval)
      await repositories.approvals.request({
        quote_id: data.quote_id,
        required_role: evaluation.requiredRole,
        requested_by: data.actor_id ?? null,
        metadata: { evaluation },
      });
  }
  const result = await workflowService.transitionQuote({
    quoteId: data.quote_id,
    toStatus: evaluation.requiredRole ? "pending_approval" : "approved",
    actorId: data.actor_id ?? null,
    payload: { action: "submit_quote_for_approval", evaluation },
...
export async function generateQuote(input: GenerateQuoteActionInput) {
  ...
  const [products, prices, approvals] = await Promise.all([
    ...
    repositories.approvals.listByQuote(data.quote_id),
  ]);
  const readiness = evaluateQuoteReadiness({
    ...
    marginPolicy: evaluateMarginFloor({
      sellPriceCents: Math.round(quote.total_amount * 100),
      costCents: 0,
      floorBps: 0,
    }),
    ...
    paymentTerms: data.payment_terms ?? (quote.metadata.payment_terms as never),
  });
  const updated = await repositories.quotes.update(data.quote_id, {
    metadata: {
      ...quote.metadata,
      readiness,
      payment_terms: data.payment_terms ?? quote.metadata.payment_terms,
      generated_at: readiness.ready ? new Date().toISOString() : null,
    },
  });
  if (readiness.ready && updated.status === "draft")
    await workflowService.transitionQuote({
      quoteId: data.quote_id,
      toStatus: "approved",
      actorId: data.actor_id ?? null,
      payload: { action: "generate_quote", readiness },
      idempotencyKey: data.idempotency_key,
    });
  await recordUpdate(updated, data.actor_id, "generate_quote", { readiness });
  revalidatePath(quotePath(data.quote_id));
  return {
    quote: await repositories.quotes.findById(data.quote_id),
    readiness,
  };
}
...
export async function sendQuote(input: SendQuoteActionInput) {
  ...
  if (quote.status !== "approved")
    throw new Error(
      `Quote ${data.quote_id} is ${quote.status} and cannot be sent.`,
    );
  const pdfUrl = `/api/quotes/${data.quote_id}/pdf`;
  const receipt = await createMockNotificationsAdapter().send({
    to: data.recipient_email,
    subject: `Quote ${quote.quote_number} is ready`,
    body: `${data.message ?? "Your quote is ready for review."}\n\nPDF: ${pdfUrl}`,
    channel: "email",
    metadata: { quote_id: data.quote_id, pdf_url: pdfUrl },
  });
  await repositories.quotes.update(data.quote_id, {
    metadata: mergeQuoteMetadata(quote.metadata, {
      metadata: {
        mock_delivery_receipt: {
          ...receipt,
...
export async function reviseRejectedQuote(
  input: ReviseRejectedQuoteActionInput,
) {
  ...
  const result = await workflowService.transitionQuote({
    ...
    toStatus: "draft",
    actorId: data.actor_id ?? null,
    payload: { action: "revise_rejected_quote" },
    idempotencyKey: data.idempotency_key,
  });
  revalidatePath(quotePath(data.quote_id));
  return result.quote;
...
export async function saveQuoteDraft(input: SaveQuoteDraftActionInput) {
  return persistRepCorrections(
    saveQuoteDraftActionSchema.parse(input),
    "save_quote_draft",
  );
}

export async function continueQuoteConfiguration(
  input: ContinueQuoteConfigurationActionInput,
) {
  const data = continueQuoteConfigurationActionSchema.parse(input);
  const { repositories, workflowService } = getContext();
  const commercialService =
    createQuoteCommercialConfigurationService(repositories);
  const result = await commercialService.configureQuoteCommercials({
    quoteId: data.quote_id,
    actorId: data.actor_id ?? null,
  });
  ...
  if (requiredRole) {
    await repositories.approvals.request({
      ...
      requested_by: data.actor_id ?? null,
      metadata: { evaluation: result.approvalEvaluation },
      idempotency_key: `configuration-${data.quote_id}-${requiredRole}`,
    });
  }
  const latest = await repositories.quotes.findById(data.quote_id);
  ...
  if (latest.status !== (requiredRole ? "pending_approval" : "approved")) {
    await workflowService.transitionQuote({
      quoteId: data.quote_id,
      toStatus: requiredRole ? "pending_approval" : "approved",
      actorId: data.actor_id ?? null,
      payload: {
        action: "continue_from_configuration",
        evaluation: result.approvalEvaluation,
        effective_discount_bps: result.effectiveDiscountBps,
        projected_margin_bps: result.calculation.grossMarginBps,
      },
      idempotencyKey:
        data.idempotency_key ?? `continue-configuration-${data.quote_id}`,
    });
  }
  revalidatePath(quotePath(data.quote_id));
  redirect(
    requiredRole
      ? `/quotes/${data.quote_id}/approval-pending`
      : `/quotes/${data.quote_id}/generate`,
  );
}
